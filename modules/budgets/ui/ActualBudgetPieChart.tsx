...
const COLORS = ["#F59E0B", "#EF4444", "#10B981", "#3B82F6", "#8B5CF6", "#4F46E5"];

interface Props {
  budgets: BudgetItem[];
}

const ActualBudgetPieChart: React.FC<Props> = ({ budgets }) => {
  const { theme } = useThemeContext();
  const chartData = useMemo(
    () => budgets.map(b => ({ name: b.category, value: b.actualAmount || 0 })),
    [budgets]
  );
  ...
  const CenterLabel = ({ viewBox }: any) => {
    ...
    const valueSize =  Number.isFinite(width) && width > 0 ? Math.min(22, width / 10) : 12;
...
export default ActualBudgetPieChart;
